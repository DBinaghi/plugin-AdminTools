<?php
	...
	class Mysqldump
	{
		...
		public function __construct(
			...
			if (count($diff) > 0) {
				throw new Exception(__('Unexpected value in dumpSettings: (%s)', implode(',', $diff)));
			}

			if (!is_array($this->dumpSettings['include-tables']) ||
				!is_array($this->dumpSettings['exclude-tables'])) {
				throw new Exception(__('Include-tables and exclude-tables should be arrays'));
			}
		...
		private function parseDsn($dsn)
		{
			if (empty($dsn) || (false === ($pos = strpos($dsn, ':')))) {
				throw new Exception(__('Empty DSN string'));
			}
			...
			if (empty($this->dbType)) {
				throw new Exception(__('Missing database type from DSN string'));
			}
			...
			if (empty($this->dsnArray['host']) &&
				...
				throw new Exception(__('Missing host from DSN string'));
			}
			$this->host = (!empty($this->dsnArray['host'])) ?
				$this->dsnArray['host'] : $this->dsnArray['unix_socket'];

			if (empty($this->dsnArray['dbname'])) {
				throw new Exception(__('Missing database name from DSN string'));
			}
			...
			try {
				switch ($this->dbType) {
					...
					case 'dblib':
						...
						break;
					default:
						throw new Exception(__('Unsupported database type (%s)', $this->dbType));
				}
			} catch (PDOException $e) {
				throw new Exception(__('Connection to %s failed with message:', $this->dbType) . $e->getMessage());
			}

			if (is_null($this->dbHandler)) {
				throw new Exception(__('Connection to %s failed', $this->dbType));
			}
		...
		public function start($filename = '')
		{
			...
			if (0 < count($this->dumpSettings['include-tables'])) {
				...
				throw new Exception(__('Table %s not found in database', $name));
			}
	...
	abstract class CompressManagerFactory
	{
		...
		public static function create($c)
		{
			...
			if (!CompressMethod::isValid($c)) {
				throw new Exception(__('Compression method (%s) is not defined yet', $c));
			}
	...
	class CompressBzip2 extends CompressManagerFactory
	{
		...
		public function __construct()
		{
			if (!function_exists("bzopen")) {
				throw new Exception(__('Compression is enabled, but bzip2 lib is not installed or configured properly'));
			}
		}
		...
		public function open($filename)
		{
			...
			if (false === $this->fileHandler) {
				throw new Exception(__('Output file is not writable'));
			}
		...
		public function write($str)
		{
			...
			if (false === $bytesWritten) {
				throw new Exception(__('Writing to file failed! Probably, there is no more free space left?'));
			}
			return $bytesWritten;
		}
	...
	class CompressGzip extends CompressManagerFactory
	{
		...
		public function __construct()
		{
			if (!function_exists("gzopen")) {
				throw new Exception(__('Compression is enabled, but gzip lib is not installed or configured properly'));
			}
		}
	...
	abstract class TypeAdapterFactory
	{
		...
		public static function create($c, $dbHandler = null, $dumpSettings = array())
		{
			...
			if (!TypeAdapter::isValid($c)) {
				throw new Exception(__('Database type support for (%s) not yet available', $c));
			}
	...
	class TypeAdapterMysql extends TypeAdapterFactory
	{
		...
		public function create_view($row)
		{
			...
			if (!isset($row['Create View'])) {
				throw new Exception(__('Error getting view structure, unknown output'));
			}
		...
		public function create_trigger($row)
		{
			...
			if (!isset($row['SQL Original Statement'])) {
				throw new Exception(__('Error getting trigger code, unknown output'));
			}
		...
		public function create_procedure($row)
		{
			...
			if (!isset($row['Create Procedure'])) {
				throw new Exception(__('Error getting procedure code, unknown output. Please check \'https://bugs.mysql.com/bug.php?id=14564\''));
			}
		...
		public function create_function($row)
		{
			...
			if (!isset($row['Create Function'])) {
				throw new Exception(__('Error getting function code, unknown output. Please check \'https://bugs.mysql.com/bug.php?id=14564\''));
			}
		...
		public function create_event($row)
		{
			...
			if (!isset($row['Create Event'])) {
				throw new Exception(__('Error getting event code, unknown output. Please check \'http://stackoverflow.com/questions/10853826/mysql-5-5-create-event-gives-syntax-error\''));
			}
			$eventName = $row['Event'];
		...
		private function check_parameters($num_args, $expected_num_args, $method_name)
		{
			if ($num_args != $expected_num_args) {
				throw new Exception(__('Unexpected parameter passed to %s', $method_name));
			}
			return;
		}
	}
